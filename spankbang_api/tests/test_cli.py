...
from spankbang_api.api import create_parser, run_main
...
class TestCLI(unittest.IsolatedAsyncioTestCase):
    ...
    def test_parser_custom_args(self):
        ...
        args = parser.parse_args([
            "--download", "https://spankbang.com/123/video/test",
            "--output", "/tmp/out",
            "--quality", "worst",
            "--no-title", "True"
        ])
        self.assertEqual(args.download, "https://spankbang.com/123/video/test")
        self.assertEqual(args.output, "/tmp/out")
    ...
    async def test_run_main_download(self):
        ...
        with patch("spankbang_api.api.Client", return_value=mock_client):
            await run_main(["--download", "https://spankbang.com/123/video/test", "--output", "/tmp/out"])

        mock_client.get_video.assert_awaited_once_with("https://spankbang.com/123/video/test", load_html=True)
    ...
    async def test_run_main_file(self):
        ...
        with tempfile.NamedTemporaryFile("w+", delete=False) as f:
            f.write("https://spankbang.com/1\nhttps://spankbang.com/2\n")
            f.flush()
            temp_name = f.name
